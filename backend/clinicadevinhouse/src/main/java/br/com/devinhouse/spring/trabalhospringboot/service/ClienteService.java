...
public class ClienteService {
    ...
    public void cadastroCliente(ClienteDTO cliente){
        if (validadorDeCadastro(cliente)) {
            ...
            throw new EntityNotFoundException("Não foi possivel cadastrar esse cliente, o CPF " + cliente.getCpf() + " já consta em nosso sistema.");
        }
    }
    ...
    private Boolean validadorDeCadastro(ClienteDTO cliente){
        ...
        for (ClienteDTO clienteDTO : clientesCadastrados) {
            if (clienteDTO.getCpf().intern() == cliente.getCpf().intern()) {return false;}
        }
        return true;
    }
    ...
    public List<ClienteDTO> procurarClientePorCpf(String cpf){
        return this.repository.findByCpfContains(cpf)
        .stream()
